...
class DataMapperDf {
  ...
  def getDataFromKafkaDf(config: Config): DataFrame = {
    val spark = SparkSession.builder.appName("HotelsBooking").getOrCreate()
  ...
  def getDataFromHdfsDf(config: Config): DataFrame = {
